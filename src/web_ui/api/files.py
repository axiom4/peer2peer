            ...
            if chunks_to_delete:
                ...
                for i in range(0, len(chunks_to_delete), batch_size):
                    ...
                    for peer in broadcast_targets:
                        tasks.append(session.post(f"{peer}/chunks/delete_batch", json=payload, timeout=5))

            if manifest_id:
                payload_manifest = {"chunk_ids": [manifest_id]}
                for peer in broadcast_targets:
                    tasks.append(session.post(f"{peer}/chunks/delete_batch", json=payload_manifest, timeout=5))
...
async def delete_manifest(request):
    ...
    if is_id:
        ...
        try:
            ...
            found_ids = [item['id'] for item in items if item.get('name') == name.replace('.manifest', '') or item.get('name') == name]
    ...
    if active_peers:
        async with aiohttp.ClientSession() as session:
            ...
            for chunk in data['chunks']:
                chunk_tasks.append(check_chunk_task(session, chunk['id'], active_peers))
...
async def download_file(request):
    ...
    task_id = str(uuid.uuid4())
    TASKS[task_id] = {"status": "processing", "percent": 0, "message": "Initializing download..."}
...
async def stream_download(request):
    ...
    try:
        result = await loop.run_in_executor(
            None, lambda: reconstruct(args, progress_callback=None, stream=True)
        )
        if not result:
...
async def repair_file(request):
    ...
    task_id = str(uuid.uuid4())
    TASKS[task_id] = {"status": "processing", "percent": 0, "message": "Starting repair..."}

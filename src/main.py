...
def main():
    parser = argparse.ArgumentParser(description="Secure P2P Storage Tool (LibP2P)")
    subparsers = parser.add_subparsers(dest="command", help="Commands")

    server_parser = subparsers.add_parser("start-node", help="Starts a storage server node")
    server_parser.add_argument("--port", type=int, default=10000, help="Port to listen on")
    server_parser.add_argument("--host", type=str, default="0.0.0.0", help="Host")
    server_parser.add_argument("--storage-dir", type=str, default="network_data/node_data", help="Data folder")
    server_parser.add_argument("--join", type=str, help="MultiAddr of peer to join")
    ...
    dist_parser.add_argument("--local", action="store_true", help="Use local simulation")
    ...
    rec_parser.add_argument("--local", action="store_true", help="Use local simulation")

    web_parser = subparsers.add_parser("web-ui", help="Start Web UI")
    web_parser.add_argument("--port", type=int, default=8888, help="Port to listen on")

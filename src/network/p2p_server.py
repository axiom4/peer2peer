...
class P2PServer:
    def initialize_p2p_callbacks(self):
        ...
        self.catalog_file = os.path.join(self.storage_dir, "network_catalog.json")
        ...
        async def on_catalog_change(data, action="update"):
             ...
             if action == "update":
                 ...
                 catalog = [item for item in catalog if item.get('id') != manifest_id]
    ...
    def __init__(self, host_ip: str, port: int, storage_dir: str, known_peer: str = None):
        ...
        self.protocol_handler = P2PProtocol(self.storage_dir, cb_change, cb_get)
    ...
    async def initialize(self):
        ...
        async def _init_trio():
            ...
            host.set_stream_handler(CHUNK_PROTOCOL, self.protocol_handler.handle_stream)
        ...
        logger.info(f"LibP2P Host initialized on /ip4/{self.host_ip}/tcp/{self.port}")
